...
export function renderAnalysis(
  ...
  for (const line of [
    ...
    '  - that ratio comes from 15 PAIRED runs (9 en, 6 fr) where the only difference',
    '    between arms was caveman, proven per run from the transcript injections. It',
    '    measures CLOSING turns. Every run was one headless prompt on one model, and',
    '    headless sessions never reach the part of the decay curve where compliance',
    '    collapses — trial compliance was 9/9 in English against 45-72% on this corpus',
    '  - mid-run R is a pilot-informed placeholder, NOT a measurement: leave-one-out on',
    '    the pilot spanned 0.56 to 1.76, straddling 1.0, so expansion is not excluded.',
    ...
    '    billed output minus visible text and tool args — which came out NEGATIVE on 2 of',
    '    16 trial pairs. Thinking is ~89% of billed output here, so if caveman does move',
    '    it, everything above is understated by a lot. Nothing in reach measures it',
